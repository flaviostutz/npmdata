# folder-publisher — Developer Notes
...
`initPublisher()` modifies the target `package.json` to include `files`, `bin`, and `dependencies` fields, then writes a thin `bin/folder-publisher.js` that calls `runner.run(__dirname)`. The generated script is kept minimal on purpose — all logic lives in this library.
